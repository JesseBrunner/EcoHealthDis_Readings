...
#| fig-cap:  "A hypothetical dose response."
#| fig-alt: "A line graph showing how the proportion of a group responding in saome way (y-axis) increases with log of the dose of something (x-axis). The line increases from a low near zero to a high near 1 in a sigmoidal pattern."
...
On the flip side, you can see why pooling samples and running them together, may not have a huge impact on the probability of detection. If, say, four swabs are run together and only one is from an infected individual, then the $d$ copies in the sample are diluted to $d/4$, but that does not matter a great deal. For instance, if the sample had $d=100$ SARS-CoV-2 particles and $\phi = 0.5$ then the probability of detection would be reduced from a high of `r 1-exp(-0.5*100)` if run by itself to a low of `r 1-exp(-0.5*100/4)` in a pool of four. That is, the pooling hardly matters. In actual fact, sensitivity can be reduce by a non-negligible amount, but the ability to screen more samples quickly can outweigh this performance cost. Pooling is most often used to screen for rare infections in large populations; the origional application was screening for syphilis in military recruits.
...
#| column: margin
#| fig-height: 6
#| fig-cap: "A hypothetical distribution of tolerances in a group of hosts (top) and the proportion of the group that would be infected (or die) if exposed to a given dose (bottom)."
#| fig-alt: "Two graphs, both with log-10 of dose along the x-axis. The top shows a bell curve and the bottom a sigmoidal curve. The middle of the bell curve aligns with the halfway point of the sigmoidal curve."

top <- ggplot(data.frame(x=0:6), aes(x=x)) + 
...
bottom <- ggplot(data.frame(x=0:6), aes(x=x)) + 
...
Now let us imagine that we study two populations with the same _average_ tolerance (or resistance) to the virus. The first is an inbred group of animals that are more alike, and thus less variable (smaller $\sigma$) while the second is a wild population of outbred animals that has much more variation among individuals (larger $\sigma$). As you would expect, the tolerance  distribution for the wild population is wider than for the inbred population, but as you might not have expected, you can actually see the outcome in a dose-response experiment (fig-tol2). The wider distribution of tolerances results in a less-steep sigmoidal curve. Why is that? Hint, pick a point on the x-axis (e.g., 2), and think about what fraction of each population would succumb. Then pick another (e.g., 4) and repeat. 

```{r}
#| label: fig-tol2
#| column: margin
#| fig-height: 6
#| fig-cap: "Tolerance distributions (top) and proportion infected at a given dose (bottom) for a highly inbred (red) and a more variable, outbred (blue) population."
#| fig-alt: "Two graphs, both with log-10 of dose along the x-axis, as in @fig-tol1. The top shows a narrow (red) and wide (blue) bell curve and the bottom a sharp (red) and shallower (blue) increasing sigmoidal curve."
#| 
top <- ggplot(data.frame(x=0:6), aes(x=x)) + 
...
#| column: margin
#| fig-height: 6
#| fig-cap: "Exponential growth of parasite populations within a host up to a lethal threshold, starting from various initial densities, on a linear (top) or logarithmic (bottom) y-axis. "
#| fig-alt: "The top graph show five lines representing parasites growing exponentially from a starting size of P=10,000 down to P=1. Because of the logarithmic y-axis,s the same lines are straight in the bottom graph. In both the lines starting from fewer parasite take longer to reach the same lethal level (here P = 1 million)."

df <- expand.grid(time=0:30, P_0 = 10^(0:4))
df <- df %>% mutate(P_t = P_0*exp(0.5*time)) 

top <- ggplot(df, aes(x=time, y=P_t, group = P_0)) + 
  geom_line() + 
...
bottom <- ggplot(df, aes(x=time, y=P_t, group = P_0)) + 
  geom_line() + 
  geom_hline(yintercept = 10^6) + 
...
* Dose-response relationship, and the underlying logic of them, emphasize the _quantitative_ nature of many aspects of infections. Reducing one's exposure to infected people by some amount (e.g., going shopping half as often) does not guarantee that one does not become infected, but it can have a measurable impact on the chances of becoming infected. Indeed, this perspective underpins the rationale for wearing masks to reduce the risk of droplet and even aerosol-borne infections: tight-fitting, effective masks reduce the number of infectious particles the wearer is exposed to, as well as the number that they shed into the environment if they are sick. Remember, though, that risk is not binary (yes/no, all or nothing) but rather continuous and we would do well to think about it that way.
