...
\begin{equation*}
\begin{aligned}
\text{Number on square 64} \rightarrow n_{64} &= 2^{64-1} \\
&= 2^{63} \\
&= \text{`r prettyNum(as.character(as.bigz(2^63)), big.mark = " ")`}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
\text{Number on all 64 squares} \rightarrow T_{64} &= 2^{64}-1 \\ 
&= \text{`r prettyNum(as.character(as.bigz(2^64-1)), big.mark = " ")`}
\end{aligned}
\end{equation*}


Those are big numbers! Quintillions or, if you prefer, billions of billions of grains of rice! But quite honestly that doesn't mean a lot to me. I tried asking [Wolfram Alpha](https://www.wolframalpha.com/input/?i=2^64+grains+of+rice) how much rice this was and while the nutrition label wasn't terribly helpful (@fig-rice), it is sort of fun!
```{r}
#| label: fig-rice
#| column: margin
#| fig-cap: "Nutritional value of a huge amount of rice."
#| fig-alt: "USDA-style nutritional label for 9*10^14 kg of rice."
...
It can be hard to recognize how a doubling process might grow in the early days. Again, the initial numbers are small and seemingly innocuous, if not plain hard to detect^[Consider the discovery of the [Asian giant hornet](https://www.nytimes.com/2020/05/02/us/asian-giant-hornet-washington.html) in the Pacific Northwest a few years ago. Was it _just_ introduced? Were there multiple introductions? Would it grow exponentially?]. But also, most processes, at least biological processes are messy or stochastic (@fig-noise). One infected person might give rise to one new infection, or three, or none. Those chance outcomes have a lot of influence early on in the process when the numbers are low, but average out later, when there are more people doing the infecting, or more individuals of an invasive species reproducing. 

```{r}
#| label: fig-noise
#| column: margin
#| warning: false
#| fig-cap: "The doubling process with increasing amounts of chance or \"noise\" in the process. Each process was run three time. Notice that one of the three processes with the most noise fizzled out after generation three."
#| fig-alt: "Nine lines increasing over time, but with slighlty different rates. One actually hits zero. "
...
Often, however, growth (or decline) does not happen in discrete steps, but rather everything happens all the time (@fig-double_exp). Think of dividing bacteria, transmission of infections, or the buying and selling of stocks. In these continuous-time cases the same logic applies, but the equation looks slightly different:

```{r}
#| label: fig-double_exp
#| column: margin
#| fig-cap: 'Geometric doubling (black line) occurs in discrete steps whereas exponential doubling (red line) is a continuous process.'
#| fig-alt: "Two lines, the exponential increasing smoothly and the geometric increasing in steps."
...
If you want to ignore the math^[But why?], just note that these two equations are equivalent and produce equivalent dynamics (@fig-double_exp); only one takes time (or chess squares) in discrete steps and the other treats it as continuous. Most mathematically-inclined people favor the exponential equation because it has nicer properties that make it easier to work with. But the logic is the same.

# Exponential means a _constant_ growth rate

In our examples so far, some quantity (rice, lily pads, infections) doubles in every $\tau$ time steps or generation. This means that every individual (grain of rice, infected individual) yields two individuals in the next $\tau$ steps, always. It doesn't matter how many there are or what else is happening; there is _always_ a doubling. This can be easier to see if we plot the data on a logarithmic scale (@fig-chess2). 

```{r}
#| label: fig-chess2
#| fig-height: 8
#| column: margin
#| fig-cap: "The first ten squares of the chess board on a linear y-axis (top),  log base 2 y-axis (middle), and log base 10 y-axis (bottom)"
#| fig-alt: "Lines increasing exponentially (top) and linearly (middle and bottom)."
...
```
What is important about a logarithmic scale is that there is a constant proportionality. On the middle graph of @fig-chess2, with a log base 2 y-axis, the distance along the y axis between 2 and 4 is the same as between 16 and 32 or 512 and 1024; each represents a doubling of the number. On the bottom graph of @fig-chess2, with the more common log base 10 y-axis, the distance between 1 and 10 is the same as between 10 and 100; each represents a 10-fold increase. 
...
#| fig-height: 6.5
#| column: margin
#| fig-cap: "Exponential growth with a range of intrinsic growth rates, $r$ on a linear scale (top) a logarithmic scale (bottom)"
#| fig-alt: "Lines growing at a faster rate in the top graph have a steeper slope on the bottom graph."
...
ggpubr::ggarrange(
  Q + scale_y_continuous("N(t)", labels=comma),
  Q + scale_y_log10("N(t)", breaks = 10^c(1:9), labels=comma), 
  ncol = 1, common.legend = TRUE
)
```

It is also important to note that slightly different slopes, representing different growth rates, can have dramatic effects on the resulting amounts (@fig-growthrates). Even seemingly small changes in slopes (i.e., $r$ or $\lambda$) can have profound impacts on how large something grows, even over just 10 time steps (e.g., several thousand compared to several hundred thousand). And in dozens or hundreds of time steps these small differences in growth are magnified into massive differences! The power of compounding!

This same less also applies to _changes_ in slopes part way through a process. When we look at cases of COVID-19 (or resulting deaths)---as I was obsessively doing early in the pandemic (@fig-growthcovid)---or any epidemic we are interested in the slope as cases go up, which indicates how rapidly the epidemic is growing. We are also looking (and hoping) for these lines to bend flatter and flatter, meaning first that the rate at which the epidemics are growing is slowing. Eventually we hope they will slope downward, meaning the epidemic is waning.  


```{r}
#| label: fig-growthcovid
#| fig-cap-location: margin
#| fig-width: 4
#| fig-cap: "Number of new confirmed cases per day. From http://91-divoc.com/pages/covid-visualization/ on 13 May 2020. The slopes in this figure change for many reasons---social distancing, reduced contacts, local epidemics running their course---but what ever the reason, a shallower line means a slower rate of growth in new cases."
knitr::include_graphics('COVID_curves.png', dpi = 500)
```
...
```{r}
#| label: fig-covidintervention
#| column: margin
#| fig-cap: "Actual and simulated numbers of COVID-19 deaths with earlier interventions. From www.nytimes.com/2020/05/20/us/coronavirus-distancing-deaths.html."
#| fig-alt: "Lines showing the cumulative number of Covid cases on a linear y-axis reaching about 65000 with no intervention, 29000 if social distancing started one week earlier, and 11000 if social distancing started two weeks earlier."

knitr::include_graphics("Intervention_COVID.png", dpi=600)
```

Similar logic applies to a growing epidemic^[Try playing with the slider in this [epidemic calculator](https://gabgoh.github.io/COVID/index.html)] or the invasion of an exotic species. Early interventions can keep exponentially growing problems smaller and more manageable. This was clear with COVID-19 epidemics (@fig-covidintervention). In nations and regions that adopted interventions like social distancing and restricting the size of public gatherings kept those epidemics from getting nearly as large as the places that waited^[Yes, there are exceptions to the pattern and many places with early successes saw subsequent epidemics when they began to open up. A discussion of why will have to wait, but the underlying pattern, that early interventions are more effective than later ones is true across the board.]. Indeed, several analyses suggested that beginning the interventions we undertook in the U.S.A. just _one week earlier_ could have cut the number of infections and death in the early parts of the pandemic _in half_^[For that wave... there have now been many.]! That means fewer people and resources are needed to combat the growing epidemic, and the risk of it getting out of hand would have been reduced. Both people and money are saved. And all because someone understood the power of exponential growth and the importance of early interventions!
