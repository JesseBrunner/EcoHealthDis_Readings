...
#| fig-cap: "From Winemiller KO (1992) Life-History Strategies and the Effectiveness of Sexual Selection. Oikos 63:318-327. Notice a) this is three-dimensional, so the grey surfaces is slanting backwards a bit and b) we can see the classical $r-k$ axis as a slice through this grey surface."
#| fig-alt: "A 3D graph with axes for fecundity (y), age of maturity (x), and juvenile survivorship (z) and a surface between them with areas labeled opportunistic, equilibrium, and periodic. Describing this well will require a conversation."
knitr::include_graphics("Winemiller_lifehistory.png", dpi=400)
```

This scheme, the "triangular life history continuum" (@fig-lifehistory) first proposed by Winemiller and Rose (1992) for fishes, helped them delineate what they called opportunistic, equilibrium, and periodic strategies. (Think about why they think these combinations of life history traits correspond to these particular environments.) When applied to other vertebrate taxa (i.e., birds, amphibians and reptiles, mammals), it suggests that each taxa uses different sectors of the possible "strategy space," with some overlap. It also shows that fishes have the most diversity in strategies and mammals the least^[They actually follow the $r$ vs. $k$ axis pretty well.].
