...
Second, notice that larger populations grow faster, period. What if the population size was $N=1,000,000,000,000$? No problem, the population grows at rate $rN=r \times 1,000,000,000,000$. Whatever $N$ is, the population grows at rate $rN$. 
...
`r newthought("A word of warning:")` Be sure tp pay attention to the axis labels. Before we were plotting $N$ against time, but now we are plotting $dN/dt$ against $N$. They show different things even though they might have similar lines.

## Population growth rates vs. per capita growth rates
So this model and graph describes the growth of the population as a whole. What does this model say about the growth of individuals^[Here we mean the average rate of births minus deaths from individuals, not a change in their sizes]? Well, if $dN/dt$ is the population growth rate, then dividing it by $N$ gives us the average growth per individual in the population (all $N$ of them). 
...
So mathematically the average growth per individual in the population, the _per capita_^[From the Latin, by heads.], is just $r$. Always. Whatever else might be happening, no matter how big or small the population, individuals reproduce (and die) on average at rate $r$. A plot of per capita growth rate against $N$ makes this clear; it always just $r$.
...
Before doing so, however, it is worth noting that every self-replicating thing---from computer viruses to real viruses to elephants---has the _potential_ to grow exponentially. Look back to the bit on doubling to remember the consequence, how rapidly we might be neck deep in elephants if their population kept growing at a constant rate^[Well, rapidly relative to the tens of millions of years modern elephants arose.]. What keeps us from being knee deep in elephants or bacteria or, well, anything? That is what much of the field of ecology is about. But I bet you can come up with a few reasonable guesses.
...
We've only added one term, $k$, to our model and we get this nice behavior where per capita growth declines as $N$ gets bigger and it equals zero when $N=k$. (If you are unsure of this, try plugging in some numbers for $N$ and $k$, maybe $k=200$ and $N=1, 10, 100,$ and $200$.) Moreover, when we've gone past the "break even" point---maybe there are more individuals than the available resources can support---individuals will die faster than they give birth to new offspring and the per capita growth rate is _negative_. 
...
As an aside, this is one version of density-dependence, where a rate, here the per capita growth rate, is influenced by population size or density.  That is, something about being around or part of a larger (or smaller) population changes something fundamental in the process. Here we are assuming that birth rates are reduced and/or death rates are increased with higher population densities, perhaps because the finite resources are divided up among more and more individuals. We will see many other examples of density-dependence in biology.
...
You will also see that the population growth rate is highest half-way to $k$. How do we explain this, when the highest per capita growth rate was when $N$ was small? Well, the per capita growth rate is highest when population size is smallest, but then again, there are very few individuals to grow the population. At much higher population sizes there are lots of individuals doing the growing, but each has a very small per capita growth rate. In the middle we get a moderate number of individuals growing moderately. 
...
For completeness more than anything, here's the equation for solution to the logistic growth model^[This is called the logistic model because it "maps on to" the [logistic function](https://en.wikipedia.org/wiki/Logistic_function), meaning this is sort of a special case of that general function. The logistic function can be used to describe many, biological processes and patterns, from increasing probability of dying given increasing doses of a toxin to the prevalence of infection in a population when individuals do not recover. It's real workhorse and one worth getting to know. ]:
...
There we have it! This characteristic sigmoidal curve of population through time up to a "carrying capacity" we called $k$, is logistic growth. It's a curve you have almost certainly seen before, but now you see the underlying logic of it and how those assumptions we made lead to these dynamics. And it is your second model of population growth! A double Woo-woo!

Do we see logistic growth in nature? You bet! In a classic set of experiments Russian ecologist G. F. Gause studied how several species of ciliates (bacteria-eating protozoans with hair-like projections) and, in other experiments, yeast grew in isolation and in combination. You can see how the logistic curves seem to capture the general dynamics of his cliliates. It's not perfect, but it's pretty good!

```{r, fig.cap="Gause's ciliates grown by themselves or in pairs with low food (half loop) or high food availability (one loop). Notice that when the food was doubled, the population size grew about twice as large."}
knitr::include_graphics("Paurelia_Pcaudatum.png", dpi=400)
```
...
I will write more in the future about how models are used, why they are wrong, and why that can be a good thing.  For now, I hope you see how we can alter a very simple model to create interesting new dynamics, how a model simply illustrates the results of the assumptions we build into it, and why even dumb models are useful. I also hope you feel like you can use these two models to make general statements about how populations might work. (It will be on the test!^[Well, not literally. There are no tests in this class...but I would like you to give this some time and attention.])
