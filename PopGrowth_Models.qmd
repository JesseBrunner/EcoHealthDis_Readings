...
>
> --- Eric Schauber & Rick Ostfeld (2002) Ecol. Appl., 12(4):1142–1162
...
#| column: margin
#| fig-height: 6
#| fig-cap: "Population size over time, starting from $N_0=1$, in the exponential model with $r=0.2$. Top graph uses a linear y-axis and the bottom a logarithmic y-axis"
#| fig-alt: "Line graph showing population size (y-axis) increasing exponentially over time (x-axis). Top graph is a j-curbve and the bottom a straigh line."

df <- tibble(t=0:30, 
             N=2*exp(0.2*t)  
         )

top <- ggplot(df, aes(t, N)) +
  geom_line(color="red") + 
  labs(x= "time (t)", 
       y="Population size (N)") 

bottom <- last_plot() + scale_y_log10()

ggpubr::ggarrange(top, bottom, ncol = 1)
```


Which describes the size of a population, $N$, starting from $N_0$ at time $t=0$, and growing at an accelerating rate (@fig-expN top). Remember, if we take the logarithm of both sides we get
$$
\log( N(t) ) = \log(N_0) + rt,
$$
Which is the equation for a line, like $y=mx+b$ with $y=\log( N(t) )$, an intercept of $\log(N_0)$ and a slope of $r$ (assuming we are using the natural logarithm).  This formulation (see @fig-expN bottom) emphasizes the constant proportionality in growth of the population. A population grows from 10 to 100 in the same amount of time it takes to grow from 100 to 1000 or 1000 to 10,000. 
...
#| label: fig-exp_dN
#| column: margin
#| fig-cap: "Population _grawth rate_  as a function of _population size_ with $r=0.2$ from the exponential model."
#| fig-alt: "A line increasing linearly indicating population growth rate (y-axis) increases linearly with population size (x-axis)."
...
```
We can plot this relationship between growth rate and population size (@fig-exp_dN). As you would expect from what we just said in words, this relationship is just a straight line starting at zero^[With a population of zero, there is no one to reproduce!] that goes up and up forever with a slope of $r$. 

**A word of warning**: Be sure to pay attention to the axis labels. Before we were plotting $N$ against time, but now we are plotting $dN/dt$ against $N$. They show different things even though they might have similar lines.
...
So mathematically the average growth per individual in the population, also known as the _per capita_^[From the Latin, by heads.] growth rate, is just $r$. Always. Whatever else might be happening, no matter how big or small the population, individuals reproduce (and die) on average at rate $r$. A plot of per capita growth rate against $N$ makes this clear; it always just $r$ (@fig-exp_dN_N).
 
```{r}
#| label: fig-exp_dN_N
#| column: margin
#| fig-cap: "Per capita growth rate as a function of population size with $r=0.2$ in the exponential model."
#| fig-alt: "A flat line at a height (per-capita growth rate) of r=0.2, no matter what the population size is (x-axis)"
...
Perhaps food or water or shelter or who knows what is limiting. Thus, when $N$ gets big enough, there is just enough resources for individuals to replace themselves^[On average. In reality, some might do really well and others poorly, but these effects balance out.], meaning the per capita growth rate (and thus population growth rate) would be zero^[Remember we are talking about growth rate, not $\lambda$, the population multiplier in a discrete time step model like you've seen elsewhere. When $\lambda=1$, per capita growth is zero.]. In a simplistic sense, there can be no (successful) reproduction unless someone dies to make room for that new individual. Let us call this population size where individuals _just_ replace themselves $k$ (blue dot in @fig-exp_dN_N_2). 

 
```{r}
#| label: fig-exp_dN_N_2
#| column: margin
#| fig-cap: "Per capita growth rate as a function of population size with $r=0.2$ in the exponential model and a point at $N=0$ where per capita growth is $r$, and one at $N=k=200$ where per capita growth is zero."
#| fig-alt: "Identical to @fig-exp_dN_N except for the two blue dots described in the caption."
...
So we know the extremes---at low $N$ per capita growth is $r$ and a $N=k$ per capita growth is zero (@fig-exp_dN_N_2)---but how do we connect these terms? We have many options. 

Three are shown in the figure to the right (@fig-exp_dN_N_3). At one extreme (line C) we could create a model where the per capita growth is unaffected by population size right up to the point where it goes to zero, like an on/off switch. This does not seem very reasonable, as just below $k$ there is no effect of population size and then add one more individual and _bam!_, no more net growth! Instead, we might say that the negative effect of population size on per capita growth rate is negligible at low population sizes and becomes larger and larger (line B). That seems somewhat more reasonable. 

```{r}
#| label: fig-exp_dN_N_3
#| column: margin
#| fig-cap: "Three ways of connecting the two blue dots in @fig-exp_dN_N_2."
#| fig-alt: "The same as @fig-exp_dN_N_2 only the two dots have been connected by a straight line (line A), a line curving down (line B), or vertical step down at N = k (line C)."
...
```{r}
#| label: fig-exp_dN_N_4
#| column: margin
#| fig-cap: "Per capita growth rate as a function of population size in the exponential model and logistic models with $r=0.2$ and $k=200$."
#| fig-alt: "A modified version of @fig-exp_dN_N_3 with only a flat line, representing the constant per-capita growth rate in the exponential model, and a straight declining line, represeneting per-capita growth rates declining linearly with population size in the logistic model."
...
As an aside, this is one version of density-dependence, where a rate, here the _per capita_ growth rate, is influenced by population size or density.  That is, _something_ about being around or part of a larger (or smaller) population changes something fundamental in the process. Here we are assuming that birth rates are reduced and/or death rates are increased with higher population densities, _perhaps_ because the finite resources are divided up among more and more individuals. But there is nothing about the biological mechanism here. You might have heard of $k$ being the "carrying capacity" of the environment, as in all of the resources available in the environment in some place, which we could count up, at least in principle. But that is not how we define $k$. There is no term in the model representing resources. All that extra meaning is applied to the the model by us. The math just requires that _per capita_ growth is zero when $N = k$. That is, we define the carrying capacity as the population size or density at which individuals _just_ replace themselves, on average.
...
$$
which maybe pulls at your memory of math classes in the past. This is the equation for a parabola. And indeed, a plot of population growth rate against population size is parabolic (@fig-exp_dN_both). 

```{r}
#| label: fig-exp_dN_both
#| column: margin
#| fig-cap: "Population growth rate as a function of population size in the exponential model and logistic models with $r=0.2$ and $k=200$."
#| fig-alt: "A graph with two lines, one showing how population growth rate (y-axis) increases linearly with population size (x-axis) in the exponential model, the other showing how growth rate increases up to a maximum N = K/2 and then decreases back to zero at N = K in the logistic model. The logistic curve is a parabola and always less than the straight line"
...
```
You will see that the population growth rate is at first going up at about the same rate as the exponential model^[And if we make $k$ really, really big, then population growth in the logistic model will follow that of the exponential model over a much wider range (@fig-exp_dN_both). Think about what happens if $k = \infty$.], but curves away from the exponential line more and more as $N$ gets bigger. That is, the larger the population gets, the less it grows exponentially. That is the consequence of how we built this model. At $N=k$ the population growth rate is zero---that is, the population is stable---and beyond that it shrinks (@fig-exp_dN_both).
...
```{r}
#| fig-alt: "Blank axes showing population size (y-axis) against time (x-axis). There is a point N=1 when time = 0, from which one might draw the rest of the trajectory."
...
$$
You'll probably never need to use this, but if you like the math^[Yay math!] you might see that as time, $t$, gets really big, the $-rt$ becomes a very large negative number, and so the $e^{-rt}$ gets closer and closer to zero. So the stuff after the one in the denominator gets smaller and smaller, no matter what, when $t$ is big. This means that $N(t \rightarrow \infty) \rightarrow k/1 = k$. We end up at $k$. Is that what you expected? You can _see_ the results in @fig-exp_N_t_both.

```{r}
#| label: fig-exp_N_t_both
#| column: margin
#| fig-height: 6
#| fig-cap: "Population size over time, starting from $N_0=1$, in the exponential and logistic models with $r=0.2$ and $k=200$ on linear (top) and logarithmic (bottom) y-axes."
#| fig-alt: "Top graph shows two lines, one growing exponentially (j-shaped) and the other logistically (i.e., sigmoidally or s-shaped) up to N = k (y-axis) over time (x-axis). The bottom graph shows the same data on a logarithmic y-axis, so the exponential line become linear and the sigmoidal logistic line becomes one that increases and saturates at a horizontal line when N = K."
...
A <- ggplot(df, aes(t, N, color=model)) +
  ...
  labs(x= "time (t)", 
       y="Population size (N)") + 
  theme(legend.position="bottom") 
top <- A + coord_cartesian(ylim=c(0,400))
bottom <- A + scale_y_log10( labels = label_log()) 
...
#| cap-location: margin
#| fig-height: 3.5
#| fig-cap: "Gause's ciliates grown by themselves or in pairs with low food (half loop) or high food availability (one loop). Notice that when the food was doubled, the population size grew about twice as large."
#| fig-alt: "A four panel figure with data points and best-fit logistic curves overlaid on them. The rows represent the abundance of the two species and the columns the amount of food provided."
