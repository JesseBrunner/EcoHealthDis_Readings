...
Now imagine we conduct an experiment where we infect animals with a pathogen of interest^[Pick your favorite!] and then track mortality  and recovery over time. Let's say everyone survives and recovers and it takes them ten days to do so, on average. Then the recovery rate would be 1/10 days or 0.1. If, instead, 80% recovered in the same time, but the others died, the recovery rate would be 0.8/10 or 0.08. So that's the average rate of recovery for an average individual. However, there might be few or many infected individuals in the $I$ box recovering, so the overall flow of individuals from the $I$ to the $R$ box is $0.8 \times I$, using the second estimate. More generally we might call the per capita recovery rate, $\gamma$, and the overall flow of individuals from $I$ to $R$ occurs at rate $\gamma I$. 
...
textplain(mid=c(0.7, 0.85), lab = expression(gamma*I), cex=1)
textplain(mid=c(0.6, 0.3), lab = expression(alpha*I), cex=1)
```


We can use the same logic for the virulence or disease-induced mortality rate. We'll call the per capita disease-induced mortality rate $\alpha$ and so the overall flow of individuals out of the infected box and into, well, a heap on the ground^[We haven't created a box for the dead individuals. Instead, we just know that animals leaving $I$ to virulence are dead.], is $\alpha I$. If, in our hypothetical experiment 20% of infected individuals died^[The proportion of infected individuals that die is the infection fatality rate. It's actually a proportion, but whatever. It is similar to the _case_ fatality (or mortality) rate, which is the proportion of individuals meeting a case definition that end of dying. Cases are always a subset of infections, so case fatality rates are generally higher than infection fatality rates.] and they did so in eight days on average, then $\alpha = 0.2/5 = 0.04$ and the overall mortality rate would be $0.04 \times I$. 

It is not essential that you are able to estimate virulence and recovery rates, but you should see that faster times and larger proportions imply faster rates. It can also be  an interesting exercise to think about what $\alpha$ might be for different sorts of infections, like influenza, which probably won't kill you, but if it does it does so quickly, and HIV/AIDS, which was very, very likely to kill you prior to effective treatments, but took years to do so. Which has higher virulence by our definition?
...
Notice that there is no way to gain new susceptible individuals, since we are not including birth or recovery without resistance (or waining immunity). They only flow out as they become infected.

The infected box gains infections at this same rate of transmission as they are lost from susceptible class---this is a flow from $S$ to $I$, after all---and then they are lost to virulence and recovery.

$$
\frac{dI}{dt} = +\beta\frac{I}{N}S - \alpha I - \gamma I.
$$

Finally, we gain individuals in the recovered/resistant box as individuals recover, but there is no way to lose them since we are not thinking about waining immunity or other forms of mortality. 
...
SIR <- odeModel( 
          main = function (time, init, parms, ...) { 
            ...
            with(as.list(parms),  {
              ...
              dS <- -beta*(I/N)*S               		          
              dI <-  beta*(I/N)*S  - gamma*I - alpha*I   
              
              dR <- gamma*I
              list(c(dS, dI, dR)) 
            })
          }, 
          parms = c(beta=10*0.05, gamma=0.08, alpha=0.04), 
...
$$
\frac{dI}{dt} = \overbrace{ \beta\frac{I}{N}S }^{gains} - \overbrace{(\alpha + \gamma) I}^{losses}
$$

```{r, fig.margin=TRUE, fig.cap="Rates of gains (solid line) and losses (dotted line) as a function of $I$ with $\\beta = 1$, and $\\alpha=0.12$ (and $\\gamma=0$ for simplicity)."}
...
We can see that the rate of losses increases linearly with $I$, with a slope of $alpha + \gamma$, but it will take a bit more to understand the gains term, or transmission. First, let us simplify things a bit and ignore the recovered group, $R$, by setting $\gamma = 0$ and shunting all of this recovery into the virulence term ($\alpha = 0.04 + 0.08 = 0.12$). Since we can ignore the $R$ class, we see that $N=S+I$ and, by rearrangment, $S=N-I$. If we place this in the gains equation and do some re-arranging we get:
...
Next, how long, on average, does a single infected individual stay infected before recovering or dying? Well, the average duration an individual spends in a box, such as the $I$ box, is one over the rate at which it leaves the box---a faster rate of loss means a shorter residence time. In this case, the rate of loss is $\alpha + \gamma$ and so the average residence time or life time of an infection is $1/(\alpha + \gamma)$. 
...
&= \frac{\beta \frac{I}{N} S}{(\alpha + \gamma)I} \\
&= \frac{\beta \frac{ \cancelto{1}{I}}{N} \cancelto{N}{S} }{(\alpha + \gamma)\cancelto{1}{I}} \\
&= \frac{\beta\cancel{\frac{N}{N}}}{\alpha + \gamma} \\
...
$$
R_0 \approx 1 + r/(\alpha + \beta),
$$
from which we get $1+`r round(coef(l)[2], 3)`/(0.04 + 0.08) = `r round(1+coef(l)[2]/(0.04 + 0.08), 3)`$. This is a pretty close estimate of the  "known" value of $R_0 = \beta/(\alpha + \gamma) = 0.5/(0.04 + 0.08) =  4.1667$.
...
# Control measures: reducing $R_0$ or $R_t < 1$

You can think about most public health interventions as aimed at reducing $R_t$ or $R_0$ so that, on average, each infected individual produces less than one new infection. When this happens, the epidemic will start to fade. (Of course this happens naturally when an infection has spread so much that it runs out of susceptible hosts---at least in situations like that described by our simple model---but usually we want to avoid that and we intervene earlier in ways that reduce $R_t \text{ or } R_0 < 1$.) Indeed you can map all interventions on to one or more components of the equation for $R_0$ or the underlying bits in our SIR model.

* **Vaccination** essentially reduces $S$ and puts those individuals into the $R$ class. This means fewer individuals that can get sick and, if you get a high enough fraction of the population vaccinated, that first infected individual will not find enough susceptibles in the population to replace itself before it dies or recovers. Indeed, there is a simple equation that describes the fraction of the population that must be vaccinated, which we call $q$, to ensure that $R_0 \leq 1$: $q \geq 1-\tfrac{1}{R_0}$. As you would expect, the higher $R_0$, the greater that fraction you must vaccinate. 
...
* **Treatment** of infections, often with chemical theraputics, can reduce the duration of an infection, as well as reduce morbidity and mortality. If individuals are infectious for less time^[It is important to note that some treatments and vaccines can reduce symptoms in a person or animal without reducing their ability to spread the infection. This is good for the individuals, sure, but it can exaserbate the spread of infection, making the epidemic worse. This is one of many potential conflicts between what is optimal for an indidividual vs for a population.], there is a shorter window for transmission, which can reduce $R_0$. 
