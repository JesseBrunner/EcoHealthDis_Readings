...
#   lossLabel = paste(labels[length(labels)]," (",losses[length(losses)]," ",unit,")",sep="")
  ...
  limTop = frInputs[1]; posTop = 0.8; maxy=0
  limBot = 0; posBot = 0.1
  if(length(inputs)>1){
    for(j in 2:length(inputs)){
      rI = max(0.07, abs(frInputs[j]/2))
      rE = rI + abs(frInputs[j])
      newPosB = posBot + rE*sin(pi/2) + 0.01
      posBot = newPosB
      arcEx = posBot - rE*sin(seq(0,pi/2,length.out=100))
      arcEy = limBot - rE*(1-cos(seq(0,pi/2,length.out=100)))
      arcIx = posBot - rI*sin(seq(0,pi/2,length.out=100))
      arcIy = limBot - rE + rI*cos(seq(0,pi/2,length.out=100))
      phiTip = pi/2 - 2*min(0.05, 0.8*abs(frInputs[j]))/(rI + rE)
  ...
  if(format!="plot"){
    ...
    plottype = switch(format,
                      ...
                      "png" = bmp("Sankey.png", width=800*((maxx+3)-minx)/(maxy-miny), height=800, unit="px",res=144)
    )
  }
  ...
  lines(c(0.1, 0, 0.05, 0, 0.8),
        c(0, 0, frInputs[1]/2, frInputs[1], frInputs[1]),
        lwd=w)

  # First input label
  inputLabel = paste(labels[1]," (",inputs[1]," ",unit,")", sep="")
  #fontsize = 1.5 #max(0.5,frInputs[1]*2.5)
  text(-0.1, frInputs[1]/2, inputLabel, pos=3, srt = 90) # try pos=4
  ...
  if(length(inputs)>1){
    for(j in 2:length(inputs)){
      ...
      ##fontsize = 2.5 #max(0.5,frInputs[j]*2.5)
      text(xText, yText, fullLabel, pos=2)
  ...
  for(i in 1:(length(losses)-1)){
    ...
    # Determine inner and outer arrow radii
    rI = max(0.2, abs(frLosses[i]))
    rE = rI + abs(frLosses[i])
    # Determine points on the internal arc
    arcIx = posTop + rI*cos(seq(3/2*pi, 7/4*pi, length.out=100))
    arcIy = limTop + rI + rI*sin(seq(3/2*pi, 7/4*pi,length.out=100))
    # Determine points on the external arc
    arcEx = posTop + rE*cos(seq(3/2*pi, 7/4*pi, length.out=100));
    arcEy = limTop + rI + rE*sin(seq(3/2*pi, 7/4*pi,length.out=100))
    # Draw internal and external arcs
    lines(arcIx, arcIy, lwd=w)
    lines(arcEx, arcEy, lwd=w)
    ...
    arX = c(max(arcEx),
            posTop + (rI + rE)/2*cos(7.25/4*pi), # max(arcEx) + 0.2*(max(arcEx)-max(arcIx)),
            max(arcIx))  #posTop + rI + c(0, frLosses[i]/2, frLosses[i])
    arY = c(max(arcEy),
            limTop + rI + (rI + rE)/2*sin(7.25/4*pi), #max(arcEy)+0.6*(max(arcIy)-max(arcEy)),
            max(arcIy))#limTop + rI + c(0, arTop,0)
    if(max(arY)>maxy){maxy = max(arY)}else{maxy=maxy}
    # Draw tip of losses arrow
    lines(arX, arY, lwd=w)

    # Draw label
    txtX = arX[2] #posTop + rI + frLosses[i]/2
    txtY = arY[2] #limTop + rI + arTop + 0.05
    fullLabel = paste(labels[i+length(inputs)]," (",losses[i]," ",unit,")", sep="")
    #fontsize = max(0.5,frLosses[i]*2.5)
    text(txtX, txtY, fullLabel, pos=4, adj = 1, srt=0)

    # Save new position of arrow top
    limTop = limTop - frLosses[i]
    # Advance to new separation point
    newPos = posTop + 0.25 + 0.25*rE #+ 0.01
  ...
  lines(c(newPos,
          newPos+max(0.04,0.8*(frLosses[length(frLosses)])),
          newPos),
        c(limTop,
          limTop-frLosses[length(frLosses)]/2,
          (limTop-frLosses[length(frLosses)])),
        lwd=w)
  ...
  text(newPos+0.05, limTop-frLosses[length(frLosses)]/2, lossLabel, pos=4) # try pos=4
